...
def _validate_inputs() -> dict[str, object]:
    ...
    return {"url": url, "max_pages": max_pages, "max_depth": max_depth, "max_discovered_urls": max_discovered, "both_devices": both}
...
def _coverage_payload(crawls: list[dict[str, object]], max_pages: int, max_depth: int, max_discovered_urls: int) -> dict[str, object]:
    ...
    for entry in all_entries:
        ...
        if status == "success": type_counts[page_type]["crawled"] += 1
        if status == "failed": type_counts[page_type]["failed"] += 1
        section_counts.setdefault(section, {"discovered": 0, "crawled": 0, "failed": 0})["discovered"] += 1
        if status == "success": section_counts[section]["crawled"] += 1
        if status == "failed": section_counts[section]["failed"] += 1
    ...
    return {
        ...
        "urls_discovered": discovered, "urls_crawled": crawled, "urls_successful": successful, "urls_failed": failed,
        ...
        "status_counts": status_counts, "page_types": type_counts, "sections": section_counts,
        "sitemap": {
...
def _coverage_html(base_html: str, coverage: dict[str, object], site_url: str) -> str:
    def esc(value: object) -> str: return html.escape(str(value if value not in (None, "") else "—"))
    ...
    manifest = "".join(f"<tr><td>{esc(e.get('url'))}</td><td>{esc(e.get('page_type'))}</td><td>{esc(e.get('section'))}</td><td>{e.get('depth', 0)}</td><td>{esc(e.get('discovery_method'))}</td><td>{esc(e.get('status'))}</td><td>{e.get('ad_signal_count', 0)}</td><td>{e.get('normalized_ad_count', 0)}</td></tr>" for e in coverage["manifest"]) or "<tr><td colspan='8'>No manifest entries.</td></tr>"
...
<table><tr><th>URL</th><th>Type</th><th>Section</th><th>Depth</th><th>Discovery</th><th>Status</th><th>Ad signals</th><th>Normalized ads</th></tr>{manifest}</table></section>"""
...
def _coverage_pdf(coverage: dict[str, object], site_url: str, path: Path) -> None:
    ...
    story.append(Table([["URLs discovered", "URLs crawled", "Successful", "Failed", "Coverage", "Max depth"], [str(coverage["urls_discovered"]), str(coverage["urls_crawled"]), str(coverage["urls_successful"]), str(coverage["urls_failed"]), f"{coverage['coverage_pct']}%", str(coverage["maximum_depth_reached"])]], colWidths=[28 * mm] * 6, style=TableStyle([("GRID", (0,0), (-1,-1), .4, colors.grey), ("BACKGROUND", (0,0), (-1,0), colors.lightgrey), ("FONTSIZE", (0,0), (-1,-1), 7)])))
    story.append(Spacer(1, 5 * mm)); story.append(Paragraph("Discovery sources", styles["Heading2"]))
    s = coverage["sitemap"]; story.append(Paragraph(f"Sitemap URLs discovered: {s['urls_discovered']}; crawled: {s['urls_crawled']}; skipped: {s['urls_skipped']}", styles["BodyText"]))
    ...
    story.append(PageBreak()); story.append(Paragraph("Complete crawl manifest", styles["Heading2"]))
    rows = [["URL", "Type", "Section", "Depth", "Discovery", "Status", "Ads"]]
    for e in coverage["manifest"]: rows.append([Paragraph(str(e.get("url", ""))[:75], styles["BodyText"]), str(e.get("page_type", "")), str(e.get("section", "")), str(e.get("depth", 0)), str(e.get("discovery_method", "")), str(e.get("status", "")), str(e.get("normalized_ad_count", 0))])
...
async def _run() -> dict[str, object]:
    cfg = _validate_inputs(); url = str(cfg["url"]); max_pages = int(cfg["max_pages"]); max_depth = int(cfg["max_depth"]); max_discovered_urls = int(cfg["max_discovered_urls"]); both = bool(cfg["both_devices"])
    data_root = Path(os.environ.get("AD_SCRAPER_DATA_ROOT", "data/runs")); out = Path("ci-output"); data_root.mkdir(parents=True, exist_ok=True); out.mkdir(parents=True, exist_ok=True)
    crawler = SiteCrawler(data_root)
    wait_ms = int(os.environ.get("WAIT_MS", "750")); timeout_ms = int(os.environ.get("TIMEOUT_MS", "30000")); max_landing_destinations = int(os.environ.get("MAX_LANDING_DESTINATIONS", "5"))
    keep_evidence = os.environ.get("AD_SCRAPER_RETAIN_SCREENSHOTS", "false").lower() == "true"
    trace = os.environ.get("AD_SCRAPER_TRACE", "false").lower() == "true"
    runtime_snapshots = os.environ.get("AD_SCRAPER_RUNTIME_SNAPSHOTS", "true").lower() == "true"
    common = dict(max_pages=max_pages, max_depth=max_depth, max_discovered_urls=max_discovered_urls, wait_ms=wait_ms, timeout_ms=timeout_ms, enrich_landing_pages=True, max_landing_destinations=max_landing_destinations, discover_sitemap=True, handle_consent=True, keep_evidence=keep_evidence, scroll_page=True, capture_runtime_snapshots=runtime_snapshots, trace=trace)
    crawls = [await crawl_site(crawler, root_url=url, device="desktop", allow_subdomains=True, **common)]
    if both: crawls.append(await crawl_site(crawler, root_url=url, device="mobile", allow_subdomains=True, **common))
    observations, run_ids = _collect_observations(data_root); intelligence = build_report_intelligence(observations); coverage = _coverage_payload(crawls, max_pages, max_depth, max_discovered_urls)
    report_root = out / "report"; report_root.mkdir(parents=True, exist_ok=True)
    ...
    main_pdf = report_root / "site-report.pdf"; main_pdf.write_bytes(render_pdf_report(observations, title=f"Ad Intelligence Site Report — {url}"))
    appendix_pdf = report_root / "coverage-appendix.pdf"; _coverage_pdf(coverage, url, appendix_pdf); _merge_pdfs(main_pdf, appendix_pdf); appendix_pdf.unlink(missing_ok=True)
    _write_json(report_root / "crawl_manifest.json", coverage)
    _write_json(report_root / "site_summary.json", {"site_url": url, "run_ids": run_ids, "observation_count": intelligence.get("observation_count", len(observations)), "campaign_count": intelligence.get("campaigns", {}).get("campaign_count", 0), "competitor_count": intelligence.get("campaigns", {}).get("competitor_count", 0), "devices": intelligence.get("devices", {}), "coverage": {k: v for k, v in coverage.items() if k != "manifest"}, "capture": {"wait_ms": wait_ms, "timeout_ms": timeout_ms, "max_landing_destinations": max_landing_destinations, "screenshots_retained": keep_evidence, "trace_retained": trace, "runtime_snapshots": runtime_snapshots}})
    _write_json(out / "scrape_result.json", {"site_url": url, "run_ids": run_ids, "status": "completed", "coverage": coverage})
    _write_json(out / "run_metadata.json", {"site_url": url, "max_pages": max_pages, "max_depth": max_depth, "max_discovered_urls": max_discovered_urls, "both_devices": both, "wait_ms": wait_ms, "timeout_ms": timeout_ms, "max_landing_destinations": max_landing_destinations, "keep_evidence": keep_evidence, "trace": trace, "runtime_snapshots": runtime_snapshots, "run_ids": run_ids, "status": "completed"})
    _write_json(out / "crawl_results.json", crawls)
    evidence_note = "Screenshots and trace evidence are not retained in this automatic report." if not keep_evidence and not trace else "Evidence is retained under data/runs/."
    (report_root / "index.html").write_text(f"<!doctype html><html lang='en'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>Ad Intelligence Site Report</title></head><body><h1>Ad Intelligence Site Report</h1><p><strong>Site:</strong> {html.escape(url)}</p><ul><li><a href='site-report.html'>HTML site report</a></li><li><a href='site-report.pdf'>PDF site report</a></li><li><a href='crawl_manifest.json'>Crawl manifest JSON</a></li></ul><p>{html.escape(evidence_note)}</p></body></html>", encoding="utf-8")
    if not run_ids: raise RuntimeError("Crawl completed without producing any result.json run")
    return {"site_url": url, "run_ids": run_ids, "coverage": {k: v for k,v in coverage.items() if k != "manifest"}}


def main() -> None:
    try: print(json.dumps(asyncio.run(_run()), indent=2))
    except Exception as exc:
        print(f"Ad Intelligence CI scrape failed: {type(exc).__name__}: {exc}"); raise SystemExit(1) from exc
