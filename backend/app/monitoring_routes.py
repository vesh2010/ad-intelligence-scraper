...
def build_monitor_router(crawler: SiteCrawler, history_store: HistoryStore, monitor_store: MonitorStore) -> APIRouter:
    ...
    async def list_monitors() -> dict[str, Any]:
        return {"monitors": monitor_store.list_targets(), "count": len(monitor_store.list_targets())}
    ...
    async def run_monitor(monitor_id: str) -> dict[str, Any]:
        ...
        if prior:
            prior_session = max(str(row.get("observed_at") or "") for row in prior)
            prior = [row for row in prior if str(row.get("observed_at") or "") == prior_session]
