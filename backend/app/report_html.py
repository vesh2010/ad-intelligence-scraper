...
def render_html_report(observations: list[dict[str, Any]], title: str = "Ad Intelligence Report") -> str:
    ...
    campaign_rows = "".join(
        ...
        f"<td>{'Yes' if row.get('competitor') else 'No'}</td>"
    ...
    competitor_rows = "".join(
        f"<tr><td><strong>{_text(row.get('competitor'))}</strong></td>"
        f"<td>{row.get('observations', 0)}</td>"
        f"<td>{row.get('observation_share_pct', 0):.1f}%</td>"
        f"<td>{row.get('campaign_count', 0)}</td>"
        f"<td>{_text(', '.join(row.get('evidence') or []))}</td>"
        f"<td>{_text(', '.join(row.get('pages_observed') or []))}</td></tr>"
...
small{{color:#666}}
.badge{{display:inline-block;border:1px solid #aaa;border-radius:999px;padding:2px 8px;font-size:12px}}
</style></head><body>
...
<div class="card">Competitors<div class="value">{intelligence['campaigns']['competitor_count']}</div></div>
...
<p><span class="badge">{intelligence['campaigns']['competitor_count']} identified</span> Competitor ads are separated from the publisher's own site using explicit advertiser/brand metadata or an external landing/destination domain. Ad-tech infrastructure domains are excluded. A domain is reported as a competitor candidate when advertiser identity is not exposed; the report does not guess a brand from OCR alone.</p>
...
<h2>Brand frequency</h2><table><thead><tr><th>Brand</th><th>Observations</th><th>Share</th></tr></thead><tbody>{"".join(f"<tr><td>{_text(row.get('brand_name'))}</td><td>{row.get('observations', 0)}</td><td>{row.get('observation_share_pct', 0):.1f}%</td></tr>" for row in intelligence['campaigns'].get('brands', [])) or "<tr><td colspan='3'>No brand metadata exposed.</td></tr>"}</tbody></table>
