...
def test_reconcile_prebid_bid_with_gpt_and_dom_evidence():
    dom = classify_dom_candidates([
        {
            ...
            "selector": "#slot-1",
            "iframe_src": None,
            "hrefs": ["https://example.com/product"],
            "image_urls": ["https://example.com/ad.png"],
            "video_urls": [],
    ...
    detection = AdDetectionResult(
        signals=[AdSignal.model_validate(dom[0])], dom_signal_count=1
    )
    runtime = [{
        "stage": "post_scroll",
        "data": {
            ...
            "prebid": {
                "bids": [{
                    ...
                    "deal_id": None,
                    "adserver_targeting": {"hb_bidder": "rubicon"},
                }],
                "winners": [{
                    "ad_unit_code": "slot-1",
                    "bidder": "rubicon",
    ...
    visual = [{
        "id": "slot-1",
        "screenshot": "data/runs/x/ad_candidates/candidate.png",
    }]
    ...
    record = records[0]
    assert record.ad_type == "gpt_slot"
...
def test_dom_only_candidate_is_preserved():
    ...
    assert len(records) == 1
    assert records[0].ad_type == "dom_candidate"
